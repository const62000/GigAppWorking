...
{
    public class DeleteJobQuestions
    {
        ...
        internal sealed class Handler(IJobQuestionRepository _jobQuestionRepository, IValidator<Query> _validator) : IQueryHandler<Query>
        {
            public async Task<BaseResult> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var result = await _jobQuestionRepository.DeleteJobQuestions(request.Ids);
                return result;
            }
        }
    }
